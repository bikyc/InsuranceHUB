...
export class EnglishCalendarComponent implements OnChanges {
 ...
 // END:Vikas: 06th Aug 20: Added for month calendar chnages.
  ...
  monthselection() {
    var fs = this.securityService.AccHospitalInfo.FiscalYearList.filter((f: FiscalYearModel) => f.FiscalYearId == this.securityService.AccHospitalInfo.CurrFiscalYear.FiscalYearId);
    this.fiscalYearId = (this.fiscalYearId == 0) ? fs[0].FiscalYearId : this.fiscalYearId;
    this.selectedFiscalYear = this.securityService.AccHospitalInfo.FiscalYearList.filter((fs: FiscalYearModel) => fs.FiscalYearId == this.fiscalYearId)[0];
  ...
  monthDate(monthData:any) {
    let year:any, monthNumber, fromDate, toDate
