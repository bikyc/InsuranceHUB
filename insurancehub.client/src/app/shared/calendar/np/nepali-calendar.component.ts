...
export class NepaliCalendarComponent {
  ...
  public hoursNo: number = 0;
  public minutesNo: number = 0;
  public amPm: string='';
  ...
  public outputToDatePicker:boolean = false;
  ...
  public inputNpDate: NepaliDate = new NepaliDate();
  ...
  public minValidDate_np: NepaliDate = new NepaliDate();
  public maxValidDate_np: NepaliDate = new NepaliDate();
  ...
  constructor(public npCalendarService: NepaliCalendarService, private coreService: CoreService,
  ...
  public yearKeyPress(event:any, nextElem: HTMLInputElement) {
  ...
  monthKeyPress(event:any, nextElem: HTMLInputElement) {
    let keycode = event.keyCode;
  ...
  dayKeyPress(event:any, nextElem: HTMLInputElement) {
    let keycode = event.keyCode;
  ...
  keyDownPressed(event:any, type: string = '', prev: HTMLInputElement, next: HTMLInputElement, currElm: HTMLInputElement) {
 ...
 // START:Vikas: 06th Aug 20: Added for month calendar chnages. 
  ...
  monthDate(monthData:any) {
    let year: number, monthNumber: number, fromDate: string, toDate: string
    ...
    year = parseInt(moment(monthData.year).format('YYYY'));
    monthNumber = +this.monthNumber;

    if (monthNumber == parseInt(moment(this.selectedFiscalYear.EndDate).format('MM'))) {
      if (moment(this.selectedFiscalYear.StartDate).format('YYYY') == year.toString()) {
        fromDate = moment(new Date(year, monthNumber - 1, parseInt(moment(this.selectedFiscalYear.StartDate).format('DD')))).format('YYYY-MM-DD');
        toDate = moment(new Date(year, monthNumber, 0)).format('YYYY-MM-DD');
      }
      else {
        fromDate = moment(new Date(year, monthNumber - 1, 1)).format('YYYY-MM-DD');
        toDate = moment(new Date(year, monthNumber - 1, parseInt(moment(this.selectedFiscalYear.EndDate).format('DD')))).format('YYYY-MM-DD');
      }
  ...
  disbaledValue(month:any)
  {
    ...
    let m = moment(new Date(year, mn, 0 as unknown as number)).format('YYYY-MM-DD');
