...
export class NepaliCalendarService {
  ...
  public maxNepYear: number = 2099;

  public minEngYear: number = 1901;
  public maxEngYear: number = 2042;
  ...
  static maxNepYear_static: number = 2099;
  ...
  static LoadNepYear_MthHash() {
    ...
    NepaliCalendarService.yr_mth_bs_static[2082] = [31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30];//pass
  ...
  IsValidNepaliDate(yyyy: any, mm: any, dd: any): boolean {
  ...
  public ConvertEngToNepDate(engDate: string): NepaliDate {
    ...
    let bsAMPM: string = "";
    if (engHRS) {
      bsHours = NepaliHours.GetAllNepaliHours().find(a => a.hoursNumber == engHRS)?.hoursNumber ?? 0;
    }

    if (engMNT) {
      bsMinutes = NepaliMinutes.GetAllNepaliMinutes().find(a => a.minutesNumber == engMNT)?.minutesNumber ?? 0;
    }
  ...
  public ConvertEngToNepaliFormatted(engDate: string = '', format: string): string {
    let npDate: NepaliDate = engDate ? this.ConvertEngToNepDate(engDate) : this.GetTodaysNepDate();
    if (engDate) {
  ...
  static ConvertEngToNepaliFormatted_static(engDate: string = '', format: string): string {
  ...
  IsNepaliDateValid(engDate: string, minDateMoment: any, maxDateMoment: any) {
