...
export class DanpheAutoCompleteComponent implements OnInit {
    ...
    isSrcArr(): boolean {
        return (this.source && this.source.constructor && this.source.constructor.name === "Array");
    }
    ...
    inputElKeyHandler = (evt: any) => {
        ...
        switch (evt.keyCode) {
            ...
            case 38: // UP, select the previous li el
                ...
                this._changeDetectorRef.detectChanges();//Manual Change detection is added in order to detect changes in the component class that impact on html.(eg: selected element in the list) 
    ...
    public SortFilteredList(keyword: string) {
        if (this.gridsort && this.gridsort.toLowerCase() != 'sortonbasicdatatype') {
            this.filteredList.sort((a, b) => {
                ...
                if (stra && regcheckbetween.test(stra)) {
                    if (strb && strb.startsWith(keyword)) {
                        return 1;
                    }
                    if (strb && strb.endsWith(keyword)) {
