...
import { BillInvoiceReturnModel } from '../../models/bill-invoice-return.model';
import { PrintTemplateType } from '../../models/print-template-type.model';
import { CustomLabelsConfig } from '../../models/custom-labels-config.dto';
import { RouteFromService } from '../../shared-services/routefrom.service';
import { ENUM_PrintingType, ENUM_PrintType } from '../../shared-enums';
...
export class Bill_Print_CreditNote_Component {
  ...
  public BillReturnUserName: string = '';
  ...
  BillCustomLabels = new CustomLabelsConfig();
  ...
  constructor(
    ...
    this.BillCustomLabels = this.coreService.GetModuleLabels('Billing');
    this.ShowQRCode = this.coreService.GetInvoiceQRConfig('return-invoice');
  ...
  ReadPrintReceiptDisplaySettingParameter() {
    ...
    if (currParam && currParam.ParameterValue) {
      ...
      if (paramValue) {
        this.PrintTemplateTypeSettings = paramValue.find(p => p.PrintType === ENUM_PrintType.returnInvoice) || {} as PrintTemplateType;
      }
    }
  }
