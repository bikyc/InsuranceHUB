...
import { finalize, Observable, of, pipe, startWith, subscribeOn, switchMap } from 'rxjs';
...
import { ENUM_INSHTTPResponseText, ENUM_MessageBox_Status, ENUM_Scheme_ApiIntegrationNames } from '../../shared/shared-enums';
import { PharmacyBLService } from '../../shared/shared-services/pharmacy-service/pharmacy.bl.service';
...
export class InsuranceBillListComponent {
  ...
  patientCodeControl = new FormControl();
  patientNameControl = new FormControl();
  filteredPatientsByMemberNo: Observable<any[]> = new Observable<any[]>;
  filteredPatientsByPatientCode: Observable<any[]> = new Observable<any[]>;
  filteredPatientsByName: Observable<any[]> = new Observable<any[]>;
  constructor(
  ...
  ngOnInit() {
    // MemberNo search
    this.filteredPatientsByMemberNo = this.memberControl.valueChanges.pipe(
      startWith(''),
      switchMap(value => value ? this.AllPatientSearchAsync(value) : of([]))
    );

    // PatientCode search
    this.filteredPatientsByPatientCode = this.patientCodeControl.valueChanges.pipe(
      startWith(''),
      switchMap(value => value ? this.AllPatientSearchAsync(value) : of([]))
    );

    // ShortName search
    this.filteredPatientsByName = this.patientNameControl.valueChanges.pipe(
      startWith(''),
      switchMap(value => value ? this.AllPatientSearchAsync(value) : of([]))
    );
  }

  // Existing async search
  public AllPatientSearchAsync = (keyword: any): Observable<Patient[]> => {
    return this.pharmacyBLService.GetPatients(keyword, false);
  }

  // Existing OnPatientSelect logic
  OnPatientSelect() {
    ...
    if (this.SelectedPatient && !this.SelectedPatient.PatientId) {
      ...
      this._messageBoxService.showMessage('Notice', ['Please select a Patient from dropdown']);
    }
  }
  ...
  CheckValidationForClaimScrubbing(): void {
    ...
    if (selectedItems && selectedItems.length) {
      ...
      if (item && selectedItems.every(a => (a.IsSelected && a.PatientId === item.PatientId && a.SchemeId === item.SchemeId && a.MemberNo === item.MemberNo && a.ClaimCode === item.ClaimCode))) {
        ...
        .pipe(finalize(() => {
          ...
          next: (res: InsHTTPResponse) => {
            if (res.Status === ENUM_INSHTTPResponseText.OK) {
              ...
              this._messageBoxService.showMessage(ENUM_MessageBox_Status.Success, [`Invoices successfully sent for claim scrubbing.`]);
            } else {
  ...
  OpenSetClaimCodePopUp(): void {
    ...
    if (selectedItems && selectedItems.length) {
      ...
      let item = selectedItems.find(a => a.IsSelected);
      if (item && selectedItems.every(a => (a.IsSelected && a.PatientId === item.PatientId && a.SchemeId === item.SchemeId))) {
  ...
  SetInvoiceNonClaimable(): void {
    ...
    this._claimManagementBLService.UpdateClaimableStatus(selectedBills, false)
      .pipe(finalize(() => {
        ...
        next: (res: InsHTTPResponse) => {
          if (res.Status === ENUM_INSHTTPResponseText.OK) {
            this._messageBoxService.showMessage(ENUM_MessageBox_Status.Success, [`Selected invoices have been successfully set as non-claimable.`]);
            this.GetBillReviewList();
          } else {
  ...
  HandleShowNonClaimableInvoiceCheckbox(event: any): void {
  ...
  HandleBillWiseCheckBox(event: any): void {
    if (event && event.currentTarget.checked) {
  ...
  HandleMainLevelCheckBox(event: any): void {
    if (event) {
  ...
  CheckClaimCode(): void {
    if (this.NewClaimCode > 0 && this.NewClaimCode.toString().length <= 14) {
      ...
      if (patientVisitId) {
        this._claimManagementBLService.CheckClaimCode(this.NewClaimCode, patientVisitId, creditOrganizationId, this.CurrentApiIntegrationName, patientId).subscribe((res: InsHTTPResponse) => {
          if (res.Status === ENUM_INSHTTPResponseText.OK) {
            if (res.Results) {
  ...
  UpdateClaimCode(): void {
    if (this.IsClaimCodeValid && this.BillListForClaimCodeAssignment.length) {
      this._claimManagementBLService.UpdateClaimableCode(this.BillListForClaimCodeAssignment, this.NewClaimCode).subscribe((res: InsHTTPResponse) => {
        if (res.Status === ENUM_INSHTTPResponseText.OK) {
  ...
  SelectInvoice(event: any, bill: ClaimBillReviewDTO): void {
  ...
  SetInvoiceClaimable(): void {
    ...
    this._claimManagementBLService.UpdateClaimableStatus(selectedBills, true)
      ...
      .subscribe({
        next: (res: InsHTTPResponse) => {
          if (res.Status === ENUM_INSHTTPResponseText.OK) {
            this._messageBoxService.showMessage(ENUM_MessageBox_Status.Success, [`Selected invoices have been successfully set as claimable.`]);
            this.GetBillReviewList();
          } else {
            this._messageBoxService.showMessage(ENUM_MessageBox_Status.Failed, [`Unable to update claimable status of the invoices.`]);
          }
  ...
  BillPreview(bill: any): void {
    this.ShowBillPreviewPage = true;
    this.SelectedBill = bill;
    this._changeDetector.detectChanges();
  }

  HideBillPreviewPage(data: any): void {
    if (data === true) {
  ...
  UpdateBillDocumentReceivedStatus(): void {
    if (this.BillListForClaimCodeAssignment.length) {
      ...
      this._claimManagementBLService.UpdateDocumentUpdateStatus(this.BillListForClaimCodeAssignment).subscribe((res: InsHTTPResponse) => {
        if (res.Status === ENUM_INSHTTPResponseText.OK) {
          this._messageBoxService.showMessage(ENUM_MessageBox_Status.Success, [`ClaimCode successfully updated.`]);
  ...
  OpenDocumentReceivedPopUp(): void {
    const selectedItems = this.BillReviewListFiltered.filter(a => a.IsClaimable && a.IsSelected);

    if (selectedItems && selectedItems.length > 0) {
      const item = selectedItems.find(a => a.IsSelected);
  ...
  getTotalPages(): number {
    const filteredLength = this.BillReviewListFiltered.length;
    return Math.ceil(filteredLength / 20) || 1;
  }
}
